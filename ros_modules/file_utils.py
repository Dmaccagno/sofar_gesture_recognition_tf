...
class FilesUtil(object):
            ...
            for file in files:
                ...
                if file_path.endswith('csv'):
                    ...
                    df = pd.DataFrame(np.genfromtxt(file_path, skip_header=True, delimiter=',',
                                                    usecols=(C.col_id_1, C.col_id_2, C.col_id_3)))
                    data.append(df)
        return data
    ...
    def save_results_to_file(error_function, filename, gesture_id):
        path = os.path.join(C.base_path, filename, 'err' + str(gesture_id) + '.p')
        f = open(path, "wb")
        pickle.dump(error_function, f)
        f.close()

    @staticmethod
    def load_result_file(filename, results_file):
        path = os.path.join(C.base_path, filename, results_file)
